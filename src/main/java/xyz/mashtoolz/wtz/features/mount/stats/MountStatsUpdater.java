...
public class MountStatsUpdater {
    ...
    public static float getMountEnergyStrength() {
        if (currentMountEnergySignature.isEmpty()) return 0.0f;
        int glyph = currentMountEnergySignature.codePointAt(0);
        int frame = Math.clamp(glyph - MOUNT_ENERGY_FULL_GLYPH, 0, MOUNT_ENERGY_EMPTY_GLYPH - MOUNT_ENERGY_FULL_GLYPH);
        return 1.0f - (float) frame / (MOUNT_ENERGY_EMPTY_GLYPH - MOUNT_ENERGY_FULL_GLYPH);
    }

    public static boolean shouldHideVanillaMountEnergyBar() {
        return WTZClient.CONFIG.mountEnergyHideVanilla && MountUtils.isMounted() && hasMountEnergyBar();
    }

    public static Text withHiddenVanillaMountEnergyBar(Text message) {
        return message;
    }

    public static boolean shouldSkipMountEnergyGlyph(Style style, int codepoint) {
        return shouldHideVanillaMountEnergyBar()
                && isMountEnergyFont(style)
                && isMountEnergyGlyph(codepoint);
    }
    ...
    private static boolean isMountEnergyFont(Style style) {
        return style.getFont().toString().contains(MOUNT_ENERGY_FONT_FRAGMENT);
    }
